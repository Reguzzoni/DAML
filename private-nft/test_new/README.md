...
```
daml build
daml codegen js .daml/dist/test_new-0.1.0.dar -o ui/daml.js
```
...
Deploying `test_new` to the hosted Daml platform
...
DAR `test_new.dar` you have just created.

To upload the UI, create a ZIP file containing all your UI assets by executing
```
daml build
daml codegen js .daml/dist/test_new-0.1.0.dar -o ui/daml.js
(cd ui && npm install && npm run-script build && zip -r ../test_new-ui.zip build)
```
...
(`test_new-ui.zip`) you have just created and publish it.

To see your deployed instance of `test_new` in action, follow the
...
- `daml.js/test_new-0.1.0/src/User.ts` (once you've generated it):
